...
describe('Round-trip tests: Document -> Markdown -> Document', () => {
  ...
  describe('Links', () => {
    it('reads link as markdown and can edit using markdown syntax', async () => {
      ...
      expect(
        requests.some(
          (r: any) => r.updateTextStyle?.textStyle?.link?.url === 'https://example.com',
        ),
      ).toBe(true)
    })

    it('can change link URL', async () => {
      const doc = createFormattedDoc([{ text: 'Click here', link: 'https://old-url.com' }])
      mockDocsClient.documents.get.mockResolvedValue({ data: doc })
      ...
      expect(
        requests.some(
          (r: any) => r.updateTextStyle?.textStyle?.link?.url === 'https://new-url.com',
        ),
      ).toBe(true)
    })
  })
  ...
  describe('Bold/Italic', () => {
    ...
    it('can remove bold from text', async () => {
      ...
      expect(requests.some((r: any) => r.updateTextStyle?.textStyle?.bold === false)).toBe(true)
    })
  })
  ...
  describe('Idempotency', () => {
    it('editing with same markdown produces same result', async () => {
      ...
      await editDoc('test-doc-id', '[here](https://example.com)', '[here](https://example.com)')
